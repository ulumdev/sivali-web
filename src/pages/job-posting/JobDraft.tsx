...
import { useDraftJobPostings } from "../../hooks/useJobPosting";
import type { JobPostingModel } from "../../models/JobPostingModel";

export default function JobPostingDraft() {
  const { jobsDraft, loading, error } = useDraftJobPostings();
  const [search, setSearch] = useState("");
  const [currentPage, setCurrentPage] = useState(1);
  const rowsPerPage = 10;

  const filteredData = jobsDraft.filter((job) => {
    const matchesSearch = job.role?.role
      ?.toLowerCase()
            ...
            {paginatedData.map((job: JobPostingModel) => (
              <tr key={job.id} className="hover:bg-indigo-50">
                ...
                <td className="px-4 py-3 text-center">
                  ...
                  >
                    {!job.isActive ? "Draft" : "Menunggu Review"}
                  </span>
                </td>
                <td className="px-4 py-3 text-center">
