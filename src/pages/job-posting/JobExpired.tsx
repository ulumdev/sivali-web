...
import { useExpiredJobPostings } from "../../hooks/useJobPosting";
import type { JobPostingModel } from "../../models/JobPostingModel";

export default function JobPostingExpired() {
  const { jobsExpired, loading, error } = useExpiredJobPostings();
  ...
  const filteredData = (jobsExpired ?? []).filter((job) => {
    ...
    return role.toLowerCase().includes(search.toLowerCase());
  });
