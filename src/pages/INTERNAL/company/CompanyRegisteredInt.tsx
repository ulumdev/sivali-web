...
import { useRegisteredCompanies } from "@/hooks/internal/useCompanyInternal";
import type { CompanyModel } from "@/models/internal/CompanyModel";
import { useNavigate } from "react-router-dom";

export default function CompanyRegisteredInt() {
  const navigate = useNavigate();
  const { companies, loading, error } = useRegisteredCompanies();
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        ...
        <div className="relative w-full md:w-1/3">
          <input
            type="text"
            placeholder="Cari nama, email, atau industri"
            value={search}
            ...
            {paginatedData.map((company: CompanyModel) => (
              <tr key={company.id} className="hover:bg-indigo-50 border-t">
                {/* <td className="px-4 py-3">
                  <img
                    src={company.companyLogoUrl || "/images/default-company.png"}
                    alt={company.name}
                ...
                <td className="px-4 py-3 text-center">
                  <button
                    onClick={() => navigate(`/internal/companies/registered/${company.id}`)}
