...
import { useRegisteredEmployee } from "@/hooks/internal/useEmployeeInternal";
import { useNavigate } from "react-router-dom";

export default function EmployeeRegistered() {
  const navigate = useNavigate();
  const { employees, loading, error } = useRegisteredEmployee();
          ...
          <tbody className="text-sm bg-white text-left">
            {paginatedData.map((w: EmployeeModel) => (
              <tr key={w.id} className="hover:bg-indigo-50 border-t">
                ...
                <td className="px-4 py-3">{w.personalInfo?.gender ?? "-"}</td>
                <td className="px-4 py-3 whitespace-nowrap">{w.personalInfo?.dateOfBirth ? formatDate(w.personalInfo.dateOfBirth) : "-"}</td>
                <td className="px-4 py-3 whitespace-nowrap">{w.lastLoginAt ? formatDate(w.lastLoginAt) : "-"}</td>
                <td className="px-4 py-3 text-center">
                  <button onClick={() => navigate(`/internal/employees/registered/${w.id}`)} className="p-1 hover:bg-gray-100 rounded">
