...
export default function CompanyDetailRegistered(): JSX.Element {
  ...
  const npwpStatus = companyDetail.companyVerificationFile?.npwpStatus ?? null;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg border p-5 flex items-start justify-between gap-4">
        ...
        <div className="flex flex-col items-end gap-3">
          ...
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center gap-2 px-3 py-1 rounded text-xs bg-amber-50 text-amber-800 border border-amber-100">Registered</span>

            <div className="flex gap-2">
              <button className="px-3 py-2 bg-white border rounded-md text-sm hover:bg-gray-50">Suspend</button>
              <button className="px-3 py-2 bg-white border rounded-md text-sm text-red-600 hover:bg-red-50">Block</button>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Warning banner */}
      <div className="bg-amber-50 border border-amber-100 text-amber-800 rounded-md p-4">
        <div className="font-medium">Verifikasi Berkas Diperlukan!</div>
        <div className="text-sm mt-1">Segera lakukan verifikasi berkas yang telah diunggah oleh perusahaan.</div>
      </div>
        ...
        <div className="mb-4">
          ...
          {companyDetail.companyVerificationFile?.npwpUrl ? (
            <div className="flex items-center gap-3">
              <a href={companyDetail.companyVerificationFile.npwpUrl} target="_blank" rel="noreferrer" className="text-blue-600">NPWP_{companyDetail.name?.slice(0,6)}.jpg</a>
              <span className={`ml-2 px-2 py-1 rounded text-xs ${npwpStatus === "PROCESS" ? "bg-amber-50 text-amber-700" : npwpStatus === "SUCCESS" ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"}`}>
                {npwpStatus ?? "Menunggu Tindakan"}
              </span>
            </div>
          ) : (
            <p className="text-gray-500">Belum ada file NPWP</p>
          )}
        </div>
      </section>
    </div>
