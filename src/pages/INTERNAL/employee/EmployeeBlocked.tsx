...
import { useBlockedEmployee } from "@/hooks/internal/useEmployeeInternal";
import { useNavigate } from "react-router-dom";

export default function EmployeeBlocked() {
  const navigate = useNavigate();
  const { employees, loading, error } = useBlockedEmployee();
          ...
          <tbody className="text-sm bg-white text-left">
            {paginatedData.map((w: EmployeeModel) => (
              <tr key={w.id} className="hover:bg-indigo-50 border-t">
                ...
                <td className="px-4 py-3">
                  {/* API sample doesn't include explicit block reason per worker.
                      Show statusGroup or fallback "-" */}
                  {w._statusGroup ?? "-"}
                </td>
                <td className="px-4 py-3 whitespace-nowrap">{w.lastLoginAt ? formatDate(w.lastLoginAt) : "-"}</td>
                <td className="px-4 py-3 text-center">
                  <button onClick={() => navigate(`/internal/employees/blocked/${w.id}`)} className="p-1 hover:bg-gray-100 rounded">
