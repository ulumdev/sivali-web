...
/**
 * Unblock confirmation modal (simplified design matching BlockCompanyModal pattern)
 */
export default function UnblockCompanyModal({
  open,
  companyId,
  companyName,
  onClose,
  ...
  const handleUnblock = async () => {
    ...
    try {
      ...
      Swal.fire({
        icon: "error",
        title: "Gagal",
        text: err?.message ?? "Gagal membuka blokir",
      });
    } finally {
      setLoading(false);
    }
      ...
      <div className="relative z-10 w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden">
        <div className="p-8 flex flex-col items-center gap-6">
          ...
          <h3 className="text-xl font-semibold text-gray-900 text-center">
            Buka Blokir {companyName ?? "Perusahaan"}
          </h3>

          {/* Description */}
          <p className="text-sm text-gray-600 text-center leading-relaxed">
            Pengguna akan mendapatkan akses untuk kembali mengakses layanan
            seperti semula
          </p>
