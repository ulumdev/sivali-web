  ...
  return (
    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 text-sm">
      ...
      <div className="flex justify-center sm:justify-end gap-1 items-center">
        <button onClick={() => onPageChange(Math.max(1, page - 1))} disabled={page === 1} className="px-2 py-1 border rounded bg-white hover:bg-gray-100 disabled:opacity-50">
        ...
        {pages.map((p, idx) =>
          p === "..." ? (
            ...
            <button key={idx} onClick={() => onPageChange(Number(p))} className={`px-3 py-1 border rounded ${page === p ? "bg-blue-600 text-white" : "bg-white hover:bg-gray-100"}`}>
              {p}
            </button>
          )
        )}

        <button onClick={() => onPageChange(total)} disabled={page === total} className="px-2 py-1 border rounded bg-white hover:bg-gray-100 disabled:opacity-50">
...
export default function TopupList() {
  ...
  // fetch transactions (server-side paging can still be used, but search will be client-side over returned items)
  const { data, loading, error } = useTransactionFetcher("topup", { page: 1, limit: 100 }); // fetch first 100 for better local search
  ...
  // filter locally on the already-fetched `data`
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold">List Transaction - Topup</h2>
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div className="relative w-full md:w-1/3">
          <input
            ...
            placeholder="Cari reference, company, user, amount, status..."
                ...
                {pageData.map((t: TransactionModel) => (
                  <tr key={t.id} className="hover:bg-indigo-50 border-t">
                    ...
                    <td className="px-4 py-3 text-center">
                      <button onClick={() => navigate(`/internal/transaction/top-up/${t.id}`)} className="p-1 hover:bg-gray-100 rounded">
